...
- **릴스 카드 그리드**: 2열 그리드
  - 각 카드: 썸네일 + 카테고리 태그(보라) + 태그(초록) + 메모 미리보기 (1줄, 말줄임)
  - 카드 클릭 시 상세 화면으로 이동
...
- PostgreSQL 기반 검색 (LIKE 또는 Full-Text Search)
- 한국어 검색 고려: pg_trgm 확장 활용 가능
...
- 실패 시 썸네일 없이 저장 (graceful fallback)
