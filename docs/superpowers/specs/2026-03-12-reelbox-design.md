...
| ORM | Prisma |
| Database (로컬) | PostgreSQL (brew install) |
| Database (배포) | Supabase 또는 Neon 무료 플랜 |
| Deployment | Vercel |
...
| 환경 | DB | 설명 |
|------|-----|------|
| 로컬 개발 | PostgreSQL (brew) | 내 맥에서 직접 실행, 호스팅 불필요 |
| Vercel 배포 | Supabase / Neon 무료 | 클라우드 DB, .env에 URL만 변경 |

- 로컬과 배포 환경의 차이는 `DATABASE_URL` 환경변수 하나로 전환
- Prisma 마이그레이션은 동일한 스키마로 양쪽 모두 적용 가능
